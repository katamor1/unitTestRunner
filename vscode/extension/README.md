...
- `TBD_*` などの未確定値を、専用タブからケース別に入力・確認・部分保存できます。
...
Quick Checkの解析結果には `reports/dependency_policy.md` / `dependency_policy.json` も含まれます。呼び出し先ごとの既定値は `real` / `stub` / `auto` です。`configured_mode` を変更した場合はQuick Checkまたは関数解析を再実行します。特定ケースだけ切り替える場合は、Workflowの `未確定項目を入力（N件）` からcanonical `test_spec.json` の `dependency_overrides` を `inherit` / `real` / `stub` に設定してから、ハーネス生成とビルドを再実行してください。マクロ、関数ポインタ、メンバ経由呼び出し、関数アドレス利用は自動書き換えせずレビュー対象になります。
...
テスト設計生成後は、`未確定項目を入力（N件）` を押して専用タブを開きます。値を入力しただけでは確認済みにならないため、内容を確認して項目ごとの `確認済み` を選び、`保存して反映` を実行します。変更した項目だけを保存でき、競合時は下書きを保持したまま最新値または下書きを選択できます。
...
| `UnitTestRunner: テスト設計を生成` | canonical `test_spec.json` と確認用のMarkdown/CSVを生成します。 |
| `UnitTestRunner: 未確定テスト項目を入力` | 未確定値とレビュー対象を専用フォームで入力・確認します。 |
...
| `test_spec.json` | テスト仕様のcanonical正本。GUI保存とハーネス生成に使用。 |
| `test_spec.md` | レビューしやすいテスト仕様表示。 |
| `test_spec.csv` | テストケースの一覧確認用。 |
...
| 未確定項目ボタンが表示されない | `reports/test_spec.json` と件数取得 | 先に `テスト設計を生成` を実行し、必要ならWorkflowを再表示します。 |
| 入力保存が競合する | revisionまたは項目fingerprint | `最新状態を読み込む` を実行し、競合項目ごとに最新値または下書きを選択します。 |
