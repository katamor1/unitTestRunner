...
- 現在開いている `.c` / `.cpp` ファイルから関数を選び、関数単位解析を開始できます。
...
Workflowパネルには、現在の設定状態、対象関数、出力workspace、現在の推奨工程、完了状態、よく使う操作が表示されます。
...
正式レビューや証跡確認で全工程を見たい場合は、パネル上部の `従来`、または簡易表示内の `従来パネルを表示` で詳細ワークフロー表示へ切り替えます。

レポートを開いて内容を確認したら、ファイルを保存すると次工程へ進みます。編集不要の場合や保存検知できない場合は、`保存済みとして確定` を押します。
