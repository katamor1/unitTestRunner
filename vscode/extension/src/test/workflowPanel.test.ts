...
import { renderWorkflowHtml, SIMPLE_WORKFLOW_ACTIONS } from '../workflow/workflowPanel';
...
describe('UnitTestRunner workflow panel view modes', () => {
  it('renders the simple panel first with a full workflow toggle', () => {
    ...
    assert.ok(SIMPLE_WORKFLOW_ACTIONS.some((action) => action.commandId === 'unitTestRunner.runFullGateForCurrentFunction'));
  });
});
