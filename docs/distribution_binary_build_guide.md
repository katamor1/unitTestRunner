...
作成日: 2026-07-05  
対象版数: v0.1  
対象配布物: Windows向け `unit-test-runner.exe`、VS Code拡張VSIX
...
VS Codeで対象Cプロジェクトを開き、settingsに以下を設定する。
...
- `UnitTestRunner: Analyze Current Function` または `UnitTestRunner: Analyze Selected Function` がCLIを起動する
...
- `unit-test-runner.exe --help`
- `unit-test-runner.exe --version`
- READMEの基本スモーク
- VS Code setting例
...
| VS CodeからCLIが見つからない | `unitTestRunner.cliPath` | exeの絶対パスを設定する |
