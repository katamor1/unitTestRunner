...
| 12 | Test Case Design Generator | レビュー可能なテストケース設計案をJSON/Markdown/CSVで生成 |
...
`source_digest`、`function_location`、`function_signature`、`global_access`、`call_report`、`coverage_design`、`boundary_equivalence_candidates`、`test_case_design` のように、中間成果物が明確に分かれている。
...
#### 3.3.4 テストケース設計案と実行可能テストの間にレビュー工程が必要

Step 12ではテストケース設計案を生成し、Step 13ではハーネス雛形へ変換する。
...
- Step 12: Test Case Design Generator
...
- グローバルアクセス、呼び出し、分岐、境界値、テストケース設計案を一望できるようにする
...
11. Step 12 Test Case Design Generator
...
テストケース設計案、スタブ雛形、期待値placeholderが出始めると、人間レビューが必要になる。
