...
| Finding | Resolution |
|---|---|
| DCR-001 finalized dossier schema divergence | `function_dossier.json` now preserves the original public dossier contract fields after Step17 finalization and adds review workflow fields alongside them. Regression coverage verifies the required schema keys and downstream `build-probe --dossier` / `generate-test-draft --dossier` compatibility. |
...
| DCR-004 operator docs and task template behind Step17/18 | README, smoke sample, and VS Code task template now document/use the finalized review flow with `--json` and `--finalize-dossier`, while keeping the Step16-compatible flow visible. |
| DCR-005 Step18 acceptance coverage representative only | VS Code adapter tests now directly cover timeout handling, missing report-path fallback warnings, command palette activation declarations, and copy-last-command contribution. |
...
```powershell
py -m unittest tests.test_dossier_finalizer_step17.DossierFinalizerStep17Tests.test_cli_finalize_prepare_review_and_analyze_function_step17
...
py -m unittest tests.test_dossier_finalizer_step17.DossierFinalizerStep17Tests.test_finalize_marks_source_mismatch_and_old_artifacts_as_stale_candidates
py -m unittest tests.test_vscode_adapter.VscodeAdapterTests.test_vscode_task_template_uses_json_and_finalized_review_flow
py -m unittest tests.test_dossier_finalizer_step17 tests.test_cli_entry_point_contract tests.test_cli_workflow tests.test_vscode_adapter
