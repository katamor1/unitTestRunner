...
- accept compiler skips only when no `gcc`, `clang`, or `cc` is available, and then require exact set equality with these four IDs/reasons: `tests.test_cli_execution_outcome.CliExecutionOutcomeTests.test_failed_fixture_execution_returns_nonzero_cli_result|host C compiler is required`, `tests.test_dependency_policy_end_to_end.DependencyPolicyEndToEndTests.test_real_default_and_case_stub_override_build_and_run_without_symbol_collisions|host C compiler is required`, `tests.test_dependency_real_source_inclusion.DependencyRealSourceInclusionTests.test_auto_stub_fallback_keeps_transitive_real_source_out_of_build|host C compiler is required`, and `tests.test_vc6_fixture_build_e2e.Vc6FixtureBuildEndToEndTests.test_default_fixture_analysis_compiles_and_links_without_mutating_product_tree|host C compiler is required`. The fourth entry was introduced on `main` by `597261950d94b04fea2672970cdb41a255c1ed8f` after the prior reconciliation anchor and is part of the base, not the Task 6 product diff. If a compiler exists, no compiler skip is permitted; hosted CI must record the real compiler path/version and must not skip;
...
### 0.7 2026-07-23 post-anchor full-gate reconciliation
...
ALLOWED_COMPILER_SKIPS = {
    ...
    "|host C compiler is required",
    "tests.test_dependency_real_source_inclusion.DependencyRealSourceInclusionTests."
    "test_auto_stub_fallback_keeps_transitive_real_source_out_of_build"
    "|host C compiler is required",
