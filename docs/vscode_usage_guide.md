...
5. テスト設計生成
6. `test_spec.json` の未確定項目を専用GUIで入力
...
<outputRoot>/<functionName>/
  ...
  reports/test_spec.md
  reports/test_spec.csv
...
| 未確定テスト項目を入力する | `UnitTestRunner: 未確定テスト項目を入力` |
...
| 未確定値を入力・確認する | `UnitTestRunner: 未確定テスト項目を入力` |
...
Workflowパネルの `未確定項目を入力（N件）` は自動でタブを開きません。利用者が明示的に開き、候補値または自由なC式を入力します。値を変更すると項目の `確認済み` は解除されるため、内容を確認してから再度選択します。`保存して反映` は変更した項目だけをcanonical `reports/test_spec.json` へ保存します。

再解析は、前回の `reports/function_dossier.json` とcanonical `reports/test_spec.json` を参照し、既存値の引き継ぎ候補や追加確認項目を整理します。旧 `reports/test_case_design.json` は後方互換aliasとしてのみ扱います。

競合時は下書きを保持し、最新値または下書きを項目ごとに明示選択します。ソースがstaleの場合は保存せず、先に現在の関数を再解析します。詳細は [テスト入力編集GUI 利用手順](test_input_editor.md) を参照してください。
...
11. テストを実行
→ BLOCKED（終了コード35）
→ reports/test_execution_blockers.md が自動表示
→ 「実行ブロック項目を開く（N件）」から再表示
→ 表示された最初の操作を実行
```

ブロッカーレポートは実際の `run-tests --run` が `blocked` になった場合だけ生成される。assertion失敗、timeout、cancel、`inconclusive` は通常の `test_execution_report` で確認する。各blocked実行の履歴は `runs/<run_id>/test_execution_blockers.json` と `.md` に残り、最新ビューは `reports/test_execution_blockers.json` と `.md` である。後続の実行がblocked以外になれば最新ビューとWorkflowボタンは消えるが、履歴は削除されない。

`run-tests --plan` はブロッカー状態を変更しない。JSONまたはMarkdownの公開・検証に失敗した場合も、終了コード35とblocked判定は維持される。その場合は未検証レポートを自動で開かず、通知と通常の実行レポートを確認する。
