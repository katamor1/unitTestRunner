...
def map_results_to_draft(
    parsed_output: Any | None,
    test_case_draft: dict[str, Any],
    ...
    if parsed_output is not None:
        draft_ids = {case.get("test_case_id") for case in test_case_draft.get("test_cases", [])}
        for test_case_id in parsed_cases:
            if test_case_id not in draft_ids:
                review_items.append(
                    ExecutionReviewItem(
                        f"REVIEW_UNMAPPED_{len(review_items) + 1:03d}",
                        "unmapped_test_output",
                        test_case_id,
                        "Runner output contains a test case that is not present in the draft.",
                        "Review runner generation and test case draft synchronization.",
                        "warning",
                    )
                )
